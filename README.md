# Многофункциональный чат
Данное приложение было разработано во время прохождения backend-практикума в компании SimbirSoft.

## Используемые технологии:
- Java 8
- Spring Framework (Spring Boot, Spring Web, Spring JPA, Spring Security, Spring DevTools, Spring Websocket)
- Thymeleaf
- Maven
- Bootstrap
- Liquibase
- YouTube API

## Скриншоты

### Главная страница
![alt text](https://github.com/xvnns/multi-functional-chat/blob/main/screenshots/1.jpg)

### Страница со списком всех участников чата 'general'
![alt text](https://github.com/xvnns/multi-functional-chat/blob/main/screenshots/2.jpg)

### Общение с чат-ботом
![alt text](https://github.com/xvnns/multi-functional-chat/blob/main/screenshots/3.jpg)

### Все пользователи
![alt text](https://github.com/xvnns/multi-functional-chat/blob/main/screenshots/3.jpg)
